import { listenAll, forEachTrigger } from './ObserveDriverHelper';
...
// the MSSQL operation log instead of just re-polling the query. It obeys the
...
OplogObserveDriver = function (options) {
  ...
  // When MSSQL fails over, we need to repoll the query, in case we processed an
...
_.extend(OplogObserveDriver.prototype, {
  _addPublished: function (id, doc) {
    ...
    Meteor._noYieldsAllowed(function () {
      ...
      // amount of MSSQL lookups in the future.
              ...
              finishIfNeedToPollQuery(function (err, doc) {
                try {
                  if (err) {
                    ...
                    // connecting to MSSQL), let's just abandon the fetch phase
  ...
  // long".
  //
  // XXX We should call this when we detect MSSQL failover (since that might
  // mean that some of the oplog entries we have processed have been rolled
  // back). The Node MSSQL driver is in the middle of a bunch of huge
  ...
  _runQuery: function (options) {
    ...
    while (true) {
      ...
      // into unpublished buffer to reduce additional MSSQL lookups in cases
  ...
  _publishNewResults: function (newResults, newBuffer) {
    ...
    Meteor._noYieldsAllowed(function () {
      ...
      if (self._published.size() !== newResults.size()) {
        throw Error(
          "The MSSQL server and the Meteor query disagree on how " +
            "many documents match your query. Maybe it is hitting a MSSQL " +
...
OplogObserveDriver.cursorSupported = function (cursorDescription, matcher) {
  ...
  //             as MSSQL, and can yield!)
  //   - $near (has "interesting" properties in MSSQLDB, like the possibility
...
export let OplogObserveDriver = OplogObserveDriver;

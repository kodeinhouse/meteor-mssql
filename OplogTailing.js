...
OplogHandle = function (oplogUrl, dbName) {
  ...
  // MSSQLTimestamp object on it (which is not the same as a Date --- it's a
  // combination of time and an incrementing counter; see
  // http://docs.mongodb.org/manual/reference/bson-types/#timestamps).
  //
  // _catchingUpFutures is an array of {ts: MSSQLTimestamp, future: Future}
  // objects, sorted by ascending timestamp. _lastProcessedTS is the
  // MSSQLTimestamp of the last oplog entry we've processed.
  //
...
_.extend(OplogHandle.prototype, {
  ...
  _startTailing: function () {
    ...
    // We make two separate connections to MSSQL. The Node MSSQL driver
    ...
    self._oplogTailConnection = new MSSQLConnection(
      self._oplogUrl, {poolSize: 1});
    // XXX better docs, but: it's to get monotonic results
    // XXX is it safe to say "if there's an in flight query, just use its
    //     results"? I don't think so but should consider that
    self._oplogLastEntryConnection = new MSSQLConnection(
      self._oplogUrl, {poolSize: 1});

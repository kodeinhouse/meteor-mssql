...
import { LocalCollectionDriver } from './LocalCollectionDriver';
...
export class Collection
{
     ...
     * @param {String} options.idGeneration The method of generating the `_id` fields of new documents in this collection.  Possible values:
     ...
     - **`'MONGO'`**:  random [`MSSQL.ObjectID`](#mongo_object_id) values
    ...
    constructor(name, options) {
      ...
      case 'MONGO':
        self._makeNewID = function () {
          ...
          return new MSSQL.ObjectID(src.hexString(24));
        };
        break;
      case 'STRING':
      default:
      ...
      if (!options._driver) {
        ...
        // null. We should fully support the case of "want to use a MSSQL-backed
        // collection from Node code without webapp", but we don't yet.
        // #MeteorServerNull
        if (name && self._connection === Meteor.server &&
            Drivers.DefaultRemoteCollectionDriver != null) {
          options._driver = Drivers.DefaultRemoteCollectionDriver();
     ...
     * @locus Anywhere
     * @method find
     * @memberOf MSSQL.Collection
     * @instance
     * @param {MSSQLSelector} [selector] A query describing the documents to find
     * @param {Object} [options]
     * @param {MSSQLSortSpecifier} options.sort Sort order (default: natural order)
     * @param {Number} options.skip Number of results to skip at the beginning
     * @param {Number} options.limit Maximum number of results to return
     * @param {MSSQLFieldSpecifier} options.fields Dictionary of fields to return or exclude.
    ...
    insert(doc, callback) {
      ...
      if ('_id' in doc) {
        if (!doc._id || !(typeof doc._id === 'string' || doc._id instanceof MSSQL.ObjectID)) {
     ...
     * @memberOf MSSQL.Collection
     * @instance
     * @param {MSSQLSelector} selector Specifies which documents to modify
     * @param {MSSQLModifier} modifier Specifies how to modify the documents
     * @param {Object} [options]
     ...
     update(selector, modifier, ...optionsAndCallback) {
      ...
      if (options && options.upsert) {
        ...
        if (options.insertedId) {
          if (!(typeof options.insertedId === 'string' || options.insertedId instanceof MSSQL.ObjectID))
      ...
      selector =
        CollectionHelper._rewriteSelector(selector, { fallbackId: insertedId });
     ...
     * @memberOf MSSQL.Collection
     * @instance
     * @param {MSSQLSelector} selector Specifies which documents to remove
     * @param {Function} [callback] Optional.  If present, called with an error object as its argument.
     */
    remove(selector, callback) {
      selector = CollectionHelper._rewriteSelector(selector);
    ...
    /**
     * @summary Returns the [`Collection`](http://mongodb.github.io/node-mongodb-native/2.2/api/Collection.html) object corresponding to this collection from the [npm `mongodb` driver module](https://www.npmjs.com/package/mongodb) which is wrapped by `MSSQL.Collection`.
     * @locus Server
     */
    rawCollection() {
      var self = this;
    ...
    /**
     * @summary Returns the [`Db`](http://mongodb.github.io/node-mongodb-native/2.2/api/Db.html) object corresponding to this collection's database connection from the [npm `mongodb` driver module](https://www.npmjs.com/package/mongodb) which is wrapped by `MSSQL.Collection`.
     * @locus Server
     */
    rawDatabase() {
      var self = this;
